...
class AichiconAdminDeactivateCommand extends Command
{
    protected static $defaultName = 'aichicon:admin:deactivate';
    /**
     * @var AdminManager
     */
    private $manager;
    ...
    {
        $this
            ->setDescription('Deactivate an admin')
            ->setDefinition(array(
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$user->isEnabled()) {
            throw new \RuntimeException(sprintf('Admin "%s" has already been deactivated.', $username));
        }
        $user->setEnabled(false);
        $this->manager->updateUser($user);
        $output->writeln(sprintf('Admin <comment>%s</comment> has been deactivated.', $username));
    }
    /**
     * @param InputInterface $input
     * @param OutputInterface $output
